...
- **`|| exit 0`**: Append to context-injection and formatting hooks so a crashed script never blocks the user. Omit on enforcement hooks (e.g., `git-guard`) that intentionally exit 2 to block tool calls — `|| exit 0` swallows all non-zero codes including exit 2.
