...
name: review
description: >-
  Structured code review using an 11-dimension framework, confidence scoring,
  and severity classification. States observable facts, not recommendations.
  TRIGGER when: user asks to review code, review a PR, review staged changes,
  or says "code review", "review this", or "what's wrong with this code".
  DO NOT TRIGGER when: user is writing code, refactoring, or debugging — those are
  creation tasks, not review tasks.
user-invocable: true
allowed-tools: Read, Grep, Glob, Bash
argument-hint: "[staged | file path | PR URL]"
---
...
5. **Read-only analysis.** This skill never modifies code. Report findings for the user to act on.
...
```
Signal: Error return from `db.query()` on line 47 is assigned to `result` but
        `result.error` is never checked. Line 48 proceeds to use `result.data`
        unconditionally.
```
...
```
### [CRIT] Error Handling: Unhandled error from token verification

**Signal**: Error return from `verifyToken()` on line 42 is not inspected.
Execution continues to line 45 which accesses `token.claims` — this will
throw if verification failed.
**Location**: `src/auth/middleware.ts:42`
**Confidence**: 95%
**Evidence**:
​```typescript
const token = verifyToken(req.headers.authorization);
// No error check — falls through to:
const userId = token.claims.sub;
​```
**Impact**: Authentication bypass — any request with an invalid token reaches
